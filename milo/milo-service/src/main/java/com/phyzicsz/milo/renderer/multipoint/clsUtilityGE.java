...
//            }
    ...
    private static ShapeInfo createSimpleFillShape(TGLight tg,ShapeInfo shape,ArrayList<ArrayList<Point2D>>polylines)
    {
        ...
        {             
            BasicStroke s=(BasicStroke)shape.getStroke();
            float[]dash=s.getDashArray();
            if(com.phyzicsz.milo.renderer.tactical.clsUtility.isClosedPolygon(tg.get_LineType())==false)
                if(com.phyzicsz.milo.renderer.tactical.clsUtility.IsChange1Area(tg.get_LineType(), null)==false)
                    return null;
            if(dash==null || dash.length<2)
                return null;   
    ...
    protected static void SetShapeInfosPolylines(TGLight tg, ArrayList<ShapeInfo> shapeInfos, Object clipBounds)
    {
        try
        {
            ...
            Boolean isClosed=com.phyzicsz.milo.renderer.tactical.clsUtility.isClosedPolygon(tg.get_LineType());
            int linetype=tg.get_LineType();
            ...
            Rectangle2D clipBounds)
    {
        ...
        try
        {
            if(com.phyzicsz.milo.renderer.tactical.clsUtility.IsChange1Area(tg.get_LineType(), null)==true)
                return ptsPoly;
...
/**
    ...
    protected static void buildHatchFills(TGLight tg, ArrayList<ShapeInfo>shapes)
    {
        try
       {
            ...
            Shape2 shape2=null;
            int index=0;
            if(com.phyzicsz.milo.renderer.tactical.clsUtility.isClosedPolygon(lineType)==false)
            {
                if(com.phyzicsz.milo.renderer.tactical.clsUtility.IsChange1Area(lineType, null)==false)
                {
                    //return;
                    switch(lineType)
                    {
            ...
            float hatchLineThickness= tg.get_LineThickness()/2;                
