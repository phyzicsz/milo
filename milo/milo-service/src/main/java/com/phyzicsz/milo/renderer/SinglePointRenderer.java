...
public class SinglePointRenderer {
    ...
    private SinglePointRenderer() {
        try {
            ...
            if (_SinglePointFont == null) {
                RendererException ex = new RendererException("SinglePointRenderer failed to initialize - SinglePointFont didn't load.", null);
                logger.error("error", ex);

            }
            if (_UnitFont == null) {
                RendererException ex = new RendererException("SinglePointRenderer failed to initialize - UnitFont didn't load.", null);
                logger.error("error", ex);
            }
            ...
            logger.error("single point renderer error", ex);
        }
    }
     ...
     * for the modifiers (not yet on the modifiers);
     *
    ...
    public void ProcessUnitSymbol(MilStdSymbol symbol, IPointConversion converter) {
        ...
        try {
            ...
            synchronized (_UnitFontMutex) {
                ...
                if (symbol.getKeepUnitRatio() && pixelSize > 0) {//symbols can range frmo 1.0 to 1.5
            ...
            if (gvFrame != null) {
                ...
                if (pixelSize > 0) {   //ideally doesn't get to this point
...
//                        symbol.getFullSymbolExtent().getWidth(), symbolBounds.getHeight());
    ...
    /**
     * Draw the affiliation modifier if appropriate. Call AFTER
     * ProcessUnitDisplayModifers as they could change where the affiliation
     * modifier goes.
     *
     * @param SymbolID
     * @param shapes
    ...
    private void ProcessUnitAffiliationModifiers(String SymbolID, ArrayList<ShapeInfo> shapes, Rectangle bounds, int symStd, Color textColor, Color textBackgroundColor) {
        ...
        try {
            ...
            if (echelonBounds != null
                    && (echelonBounds.width + echelonBounds.getX() > bounds.getX() + bounds.getWidth())) {// if echelon, position affiliation modifier so it doesn't overlap.
...
//                                    echelonBounds.getWidth()+2, echelonBounds.getHeight()+2);
                ...
                if (SymbolUtilities.hasInstallationModifier(SymbolID)) {//the actual installation symbols have the modifier
    ...
    private ShapeInfo CreateTaskForceIndicator(String symbolID, Rectangle2D symbolBounds, Rectangle2D echelonBounds) {
        ...
        try {
            ...
            logger.error("render task force error", ex);
        }
        return siTaskForce;
    }
    ...
    private ShapeInfo CreateInstallationIndicator(String symbolID, Rectangle2D symbolBounds) {
        ...
        try {
            ...
            logger.error("error installtion render", ex);
        }
    ...
    private ArrayList<ShapeInfo> CreateOperationalConditionIndicator(String SymbolID, Rectangle2D symbolBounds, double pixelSize) {
        ...
        try {
            ...
            logger.error("render error creating installation",ex);
        }
        return bars;
    }
    ...
    private ShapeInfo CreateEchelonShapeInfo(String echelon, FontRenderContext frc, Rectangle bounds, Color textColor, Color textBackgroundColor) {
        ...
        try {
            ...
            logger.error("error rendering echelon", ex);
        }
     ...
     *
     * @param mobility like SymbolID.substring(10, 12) where SymbolID is a 15
     * character String.
    ...
    public void ProcessSPSymbol(MilStdSymbol symbol, IPointConversion converter) {
        try {
                ...
                if (dom != null && !dom.equals("")) {
                    if (SymbolUtilities.isNumber(dom)) {
                        if (SymbolUtilities.hasDirectionOfMovement(symbol.getSymbolID(), symStd)) {
                            ...
                            if (modifiers.containsKey(ModifiersTG.Y_LOCATION)) {//if Y exists, we have to move DOM arrow down below it.
            ...
            logger.error("error processing sps symbol", ex);
        }
    }

    /**
     * Returns an arraylist representing the ShapeInfo objects used to draw all
     * of the modifiers.
     *
     * @param symbol
     * @return
     ...
     *
     * @param milStdCode
     * @param validModifiers null or "" will result in no modifiers being
     * processed
     * @param modifiers
     * @param bounds
     * @param frc
    ...
    private static ArrayList<ShapeInfo> ProcessSPTGModifiers(String milStdCode,
        ...
        try {
            ...
            logger.error("error processing modifier", ex);
        }
        return modifierShapes;
    }

    /**
     * Creates and positions a shape for the modifier. (Label Modifiers, not
     * Graphic modifiers)
     *
     * @param symbolID
     * @param modifierValue
     ...
     * @param labelFont
     * @param bounds
     * @param byLabelHeight position by label height (true) or position
     * accurately (false). Typically set to true when label font is too big to
     * position modifiers accurately.
     * @return
     */
    private static ArrayList<ShapeInfo> GetSPTGModifierShape(String symbolID, String modifierValue, String modifierName, FontRenderContext frc, Font labelFont, Rectangle bounds, Color TextColor, Color textBackgroundColor) {
        ...
        try {
            if (modifierValue != null && modifierValue.equals("") == false) {
                ...
                } else if (modifierName.equals(ModifiersTG.T_UNIQUE_DESIGNATION_1))//T
                {
                    ...
                    y = bounds.y + labelHeight;
                        ...
                        && (basicID.equals("G*O*ES----****X")
                        || //emergency distress call
                        basicID.equals("G*S*PP----****X")
                        || //medevac pick-up point
            ...
            logger.error("error rendering modifier shape", ex);
            return null;
        }
    ...
    /**
     * Creates and positions a shape for the modifier. (Label Modifiers, not
     * Graphic modifiers) Specific to symbols that fall under
     * SymbolUtilities.isTGSPWithSpecialModifierLayout()
     *
     * @param symbolID
     * @param modifierValue
     * @param modifierName example: MilStdSymbol.ADDITIONAL_INFO_1
    ...
    private static ArrayList<ShapeInfo> GetSPTGSpecialModifierShape(String symbolID, String modifierValue, String modifierName, FontRenderContext frc, Font labelFont, Rectangle bounds, Color textColor, Color textBackgroundColor) {
        ...
        try {
            if (modifierValue != null && modifierValue.equals("") == false) {
                ...
                } else if (basicSymbolID.equals("G*G*DPT---****X")
                        || //T (target reference point)
                        basicSymbolID.equals("G*F*PTS---****X")
                        || //t,h,h1 (Point/Single Target)
                ...
                } else if (basicSymbolID.equals("G*M*NZ----****X")
                        ||//ground zero
                        basicSymbolID.equals("G*M*NEB---****X")
                        ||//biological
                        basicSymbolID.equals("G*M*NEC---****X"))//chemical
    ...
    private void RotateShapeInfo(ArrayList<ShapeInfo> shapes, double angle, double anchorX, double anchorY) {
        try {
            ...
            logger.error("error rotating shape", ex);
            //return null;
        }
    }
    ...
    private ArrayList<ShapeInfo> CreateDOMArrow(String symbolCode, Rectangle bounds, double angle) {
        ...
        ArrayList<ShapeInfo> returnVal = new ArrayList<>();
     ...
     * @param labelFont
     * @param bounds
     * @param echelonBounds optional - echelon width may indicate modifiers need
     * to be further out from the symbol (Region)
     * @param affiliationBounds optional - Like in the case of Joker, some
     * modifiers may need to be repositioned.
     * @return
     */
...
//            y4 = bounds.y - (labelHeight + (int)bufferText) * 4;
            ...
            logger.error("error getting unit modifier", ex);
        }
...
//                x = symbolBounds.getCenterX() - centerOffset - (labelBounds1.getWidth()/2);
            ...
            logger.error("error creating text", ex);
        }
