...
import com.phyzicsz.milo.renderer.info.LineInfo;
import com.phyzicsz.milo.renderer.info.PNGInfo;
import com.phyzicsz.milo.renderer.info.SymbolInfo;
import com.phyzicsz.milo.renderer.info.TextInfo;
...
import com.phyzicsz.milo.geo.kml.KmlOptions;
import com.phyzicsz.milo.geo.kml.XsltCoordinateWrapper;
...
//            System.out.println("kml offset: " + String.valueOf(imageOffset) + " " + lineFill.toString());
    ...
    private static Boolean normalizePoints(ArrayList<Point2D.Double> shape, IPointConversion ipc) {
        ...
        for (int j = 0; j < shape.size(); j++) {
            ...
            geoCoord = com.phyzicsz.milo.renderer.MultiPointHandler.NormalizeCoordToGECoord(geoCoord);
            double latitude = geoCoord.getY();
            double longitude = geoCoord.getX();
            Point2D pt2d = new Point2D.Double(longitude, latitude);
            geoCoords.add(pt2d);
        }
        Boolean normalize = com.phyzicsz.milo.renderer.MultiPointHandler.crossesIDL(geoCoords);
        return normalize;
    }
